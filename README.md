...
func main() {
	client := omnistack.NewClient(
		option.WithAPIKey("My API Key"), // defaults to os.LookupEnv("OMNISTACK_API_KEY")
	)
...
```go
params := FooParams{
	Name: omnistack.F("hello"),

	// Explicitly send `"description": null`
	Description: omnistack.Null[string](),

	Point: omnistack.F(omnistack.Point{
		X: omnistack.Int(0),
		Y: omnistack.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: omnistack.Raw[int64](0.01), // sends a float
	}),
}
```
...
client.Completions.New(context.TODO(), ...,
	// Override the header
...
`*omnistack.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/completions": 400 Bad Request { ... }
}
```
...
We also provide a helper `omnistack.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   omnistack.F("id_xxxx"),
    Data: omnistack.F(FooNewParamsData{
        FirstName: omnistack.F("John"),
    }),
}
